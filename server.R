...
shinyServer(function(input, output, session) {
  ...
  update_function <- reactive({
    if(is.null(input$base_file)) {new_dat <- NULL} else {
      update_path <- input$base_file
      
      file.rename(update_path$datapath,
                  paste(update_path$datapath, ".xlsx", sep=""))
      
      new_dat <- suppressWarnings(read_excel(path = paste(update_path$datapath, ".xlsx", sep=""),col_names = TRUE,
                                             col_types = c(rep("text",14),rep("numeric",3),"date","text","text","numeric","numeric","text",rep("skip",10))))
  ...
  output$download_file <- renderUI({
    if(!is.null(update_function())) downloadButton(outputId = "download_new_dat",label = "Download")  
  })
  ...
  output$download_new_dat <- downloadHandler(
    ...
    content = function(file){
      xlsx::write.xlsx(file = file,x = data.frame(update_function()),sheetName = "data",row.names = FALSE,showNA = FALSE)
    }
  ...
  datChanges <- reactive({
    ...
    ind <- c(dat$SuspectedError %in% errorFilter) & dat$`Main constituent for DB` %in% ConstFilter & dat$`Type for DB` %in% typeFilter
  ...
  observeEvent(input$update,{
    ...
    updateCheckboxGroupInput(session,inputId = "selectCountry",
                             selected = dat$Country[dat$Country %in% tabeuc$Country])
    
    updateCheckboxGroupInput(session,inputId = "Reg",
                             selected = dat$Region[dat$Region %in% tabeuc$Region])
  ...
  output$obsGraph1 <- renderPlotly({
    ...
    plt1 <- add_trace(p = plt1,name= "New Obs",data = obs,x = ~X3,y = ~X1, mode = "markers",
                      color = "black",marker = list(color = "black",size = 5))
  ...
  output$obsGraph3d <- renderPlotly({
    ...
    plt3 <- plot_ly(d, x = d$`206Pb/204Pb`, y = d$`208/204Pb`,z = d$`207/204Pb`,
      ...
      layout(autosize = F,
             margin = list(l = 20,r = 20,b = 20,t = 20,pad = 10),
             legend = list(x = 0, y = 1,font = list(size = 14), legendgroup = "Country", orientation = "h"))  # Added - marker's place and direction
    
    plt3 <- add_markers(p = plt3,x = obs[,4],y = obs[,2],z = obs[,3],
                        type = "scatter3d",text = paste(obs$ID), color = "New obs", marker = list(color = "black",size = 5))
  ...
  output$regGraph3d <- renderPlotly({
    ...
    plt3 <- plot_ly(d, x = d$`206Pb/204Pb`, y = d$`208/204Pb`,z = d$`207/204Pb`,
      ...
      layout(autosize = F,
             margin = list(l = 20,r = 20,b = 20,t = 20,pad = 10),
             legend = list(font = list(size = 14), legendgroup = "Region"))  # Added - marker's place and direction
